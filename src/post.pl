%% no optimization -------------------------------------------------------------------------------------
q_nominimize :- 
	query_nominimize(Vars, ResCs, ResPs, Probs, Covs),
	length(ResCs, LResCs),
...
%% optimization ----------------------------------------------------------------------------------------
q_minimize :- 
	query_minimize(Vars, ResCs, ResPs, Probs, Covs, Min),
	length(ResCs, LResCs),
	lengthsublist(ResPs, LResPs),
	print(Vars), nl,
	print(ResCs), nl,
...
query_nominimize(Vars, ResCs, ResPs, Probs, Covs) :-
	...
	findall(inst(I), data_instance(_, I, _, _, _, _), Is), !,
	solve(project(satisfiable(cross([unp(typec), userc | Is])), LVars), Vars, ResCs, _, Extras),
	extract(Extras, LVars, ResPs, Probs, Covs).
...
solve(inst(I), Vars, R, [], [(R, L, P, C)]) :-
	data_instance(N, I, L, Pr, Cv, M),
	nth0(N, Vars, V),
	path(M, V, R, L, P, C),
	P >= Pr, C >= Cv.
...
extract([], _, [], [], []).
extract([(A, _, P, C)|Es], LVars, [Con|As], [P|Ps], [C|Cs]) :-
	project(LVars, A, Con),
	extract(Es, LVars, As, Ps, Cs).
...
exp_eval(l1norm(I1, I2), Vars, S, Cs) :- !,
	data_instance(N1, I1, _, _, _, _),
	data_instance(N2, I2, _, _, _, _),
	nth0(N1, Vars, INST1),
	nth0(N2, Vars, INST2),
...
exp_eval(l1normd(I1, [I2Head|I2Tail]), Vars, S + STail, NewCs) :- !,
	data_instance(N1, I1, _, _, _, _),
	% head
	data_instance(N2List, I2Head, _, _, _, _),
	nth0(N1, Vars, INST1),
