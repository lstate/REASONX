...
def getContents(filename, n=None):
    """ Get first n lines from a text file or whole contents """
...
def get_att(item, sep='=', ifnotfound=''):
    """ Extract attribute name from attribute=value string """
    pos = item.find(sep)
    return item[:pos] if pos>=0 else ifnotfound
 
def get_val(item, sep='=', ifnotfound=''):
    """ Extract attribute name from attribute=value string """
    pos = item.find(sep)
    return item[pos+1:] if pos>=0 else ifnotfound
 
class Encode:
    """ Encoding of discrete attributes in dataframes """
    def __init__(self, nominal=[], ordinal=[], decode=dict(), onehot=False, prefix_sep="="):
        """ Constructor 
        
        Parameters:
        nominal (iterable): nominal attributes to encode
        ordinal (iterable): ordinal attributes to encode
        decode (dictionary): pre-set encodings to extend  
        onehot: use one-hot-encoding for nominal (if False, map ordered values to integers)
        """
    ...
    def transform(self, df, unprocess=set()):
        ...
        if self.onehot:
            ...
            for col in cols & self.nominal:
                ...
                not_encoded = set(self.encode[col].keys())-set(df[col].unique())
                for v in not_encoded:
                    dummies[col+self.prefix_sep+v] = False
    ...
    def encoded_atts(self, cols, value=False):
        """ econded atts of a given attribute """
